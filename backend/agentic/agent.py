...
EXTRACTION_PROMPT = """اقرأ كل ما هو مكتوب في هذه الصورة واستخرج كل حقل مع قيمته.

التنسيق المطلوب:
اسم_الحقل: القيمة [HIGH/MEDIUM/LOW]

قواعد:
- اقرأ كل نص تراه في الصورة
- اكتب اسم الحقل كما يظهر في النموذج
- اكتب القيمة المكتوبة بجانبه
- إذا الحقل فارغ اكتب: ---
- إذا يوجد توقيع: [توقيع موجود]
- إذا يوجد ختم: [ختم موجود]
- HIGH = واضح، MEDIUM = مقروء، LOW = غير واضح

مثال:
الاسم: محمد أحمد [HIGH]
رقم_الهوية: ١٠٥٠٢٣٤٥٦٧ [HIGH]
التاريخ: ١٤٤٥/٠٣/١٥ [MEDIUM]
الحقل_الفارغ: --- [HIGH]

الآن اقرأ الصورة واستخرج كل الحقول:"""
...
class AgenticOCRAgent:
    ...
    async def process(self, image: Image.Image, user_context: Optional[str] = None) -> Dict[str, Any]:
        ...
        log(f"[Agent] Raw response preview:\n{result.text[:1000] if result.text else 'EMPTY'}")
        
        # Step 3: Parse response
        fields, confidence = self._parse_response(result.text)
        log(f"[Agent] Parsed {len(fields)} fields: {list(fields.keys())[:10]}...")
